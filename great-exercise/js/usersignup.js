...
if (localStorage.getItem("teachers") == "") {
  ...
  teachers[now] = JSON.parse(localStorage.getItem("teachers"))[now];
}
...
function signUp() {
  ...
  if (confirmPassWord == "") {
    ck3 = 1;
    document.getElementById("sign-up__confirm-password-null").style.display =
      "block";
  } else {
    ck3 = 0;
    document.getElementById("sign-up__confirm-password-null").style.display =
    ...
    if (confirmPassWord != passWord && passWord != "") {
      ck4 = 1;
      document.getElementById("sign-up__confirm-password-false").style.display =
        "block";
    } else {
      ck4 = 0;
      document.getElementById("sign-up__confirm-password-false").style.display =
    ...
    for (var i = 0; i < teachers.length; i++) {
      for (var j = 0; j < teachers[i].students.length; j++) {
        if (
          ...
          alert(
            "Tên đăng nhập và mật khẩu đã được sử dụng. Vui lòng đăng kí lại!"
          );
          ck = 1;
          j = teachers[i].students.length;
          i = teachers.length;
          break;
        }
      }
...
function saveTeacher() {
  teachers[index].name = document.getElementById("fullname").value;
  localStorage.setItem("teachers", JSON.stringify(teachers));
...
if (teachers.length > 0) {
  ...
  if (document.getElementById("fullname")) {
    document.getElementById("fullname").value = now.name;
  }
  ...
  for (var i = 0; i < soSv; i++) {
    if (now.students[i].scoresTb1[6] < 1) soSvTamNgungHoc++;
  }
  if (document.querySelector(".status-warning h3"))
    document.querySelector(
      ".status-warning h3"
    ).innerText = `${soSvTamNgungHoc}`;
  // điểm kthp nhỏ hơn 4
  var soSvNoMon = 0;
  for (var i = 0; i < soSv; i++) {
    if (
      now.students[i].scoresTb2[2] < 4 ||
  ...
  if (soSv > 0) {
    ...
    if (document.querySelector(".card table tbody"))
      document.querySelector(".card table tbody").innerHTML =
        innerHTMLListStudentTop;
    // điểm trên 3.6
    var soSvTren36 = 0;
    for (var i = 0; i < soSv; i++) {
      if (now.students[i].scoresTb1[6] >= 3.6) soSvTren36++;
    }
    ...
    for (var i = 0; i < soSv; i++) {
      if (
        now.students[i].scoresTb1[6] >= 3.2 &&
        now.students[i].scoresTb1[6] < 3.6
      )
        soSvTren32++;
    }
    ...
    for (var i = 0; i < soSv; i++) {
      if (
        now.students[i].scoresTb1[6] >= 2.5 &&
        now.students[i].scoresTb1[6] < 3.2
      )
        soSvTren25++;
    }
    ...
    var soSvDuoi25 = 0;
    for (var i = 0; i < soSv; i++) {
      if (now.students[i].scoresTb1[6] < 2.5) soSvDuoi25++;
    }
    ...
    if (document.querySelector(".list-student-tbody"))
      document.querySelector(".list-student-tbody").innerHTML =
        innerHTMLListStudent;
  } else {
  }
}
