...
function saveInput() {
    ...
    for (var i = 0; i < ipScore.length / 2;i++){
        ...
        SV.scoresTb2[i * 3 + 2] = Math.round((SV.scoresTb2[i * 3 + 0] + SV.scoresTb2[i * 3 + 1]) / 2 * 100) / 100;
    };
    ...
    SV.scoresTb1[10] = SV.scoresTb1[11] = Math.round((SV.scoresTb1[8] * 8 + diem2 * 10) / 18 * 100) / 100;
    SV.scoresTb1[2] = SV.scoresTb1[3] = Math.round(((SV.scoresTb1[0] * 19 + diem * 21) / 40) * 100) / 100;

    for (var i = 4; i < 8; i++){
        SV.scoresTb1[i + 8] = Math.round((SV.scoresTb1[i + 8 - 4] / 2.5) * 100) / 100;
...
function acceptLeave(index) {
    if (index) {
        var isLv = 1;
        if (!isSave) {
            isLv = confirm("Điểm chưa được cập nhập. Bạn có chắc muốn lưu lại không?");
            if (isLv) {
                saveInput();
            }
